...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 [API] GET /api/pix/check-payment-by-order - Iniciando...')
    ...
    console.log('📊 [API] Verificando pago PIX para orden:', orderId)

    // Buscar el pago PIX asociado a la orden
    ...
    if (!pixPayment) {
      return NextResponse.json({ 
        error: 'Pago PIX no encontrado para esta orden' 
      }, { 
        status: 404,
        headers: corsHeaders
      })
    ...
    console.error('❌ [API] Error checking PIX payment by order:', error)
    return NextResponse.json({ 
