...
// URLs de imágenes reales de Unsplash para cada categoría
const productImages = {
  smartphones: [
...
// Productos por categoría (4-5 productos por categoría para llegar a 60)
...
async function createProductsWithImages() {
  try {
    console.log('🚀 Creando 60 productos con imágenes reales...')
    ...
    for (const category of categories) {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        const imageUrl = images[i % images.length] // Rotar imágenes si hay más productos que imágenes
        ...
        try {
          ...
          await prisma.productImage.create({
            data: {
              path: imageUrl,
              filename: `product_${createdProduct.id}.jpg`,
              alt: product.title,
              order: 0,
